...
This blog post announces the release of [`{appsheet}`](https://calderonsamuel.github.io/appsheet/) 0.1.0 to the world. It allows to interact with Google's Appsheet API to read, add, edit and delete data from AppSheet tables.

Here, I'll explain the core functionality. But first, let's understand why no-code apps might be important for an R user.
...
That's why this package is created. I wanted a way to interact with Appsheet's API with my favorite language, taking advantage of the power of Quarto and the tidyverse for data cleaning, visualization and reporting. Now, anyone who wants to do the same thing doesn't need to start from scratch.
...
Next, you need a quarto document with the contents of your report to be rendered. Inside this report is where you should use `{appsheet}` and your favorite cleaning and visualization packages. 
